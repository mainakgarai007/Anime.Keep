...
async function fetchJikan(endpoint, fallback = { data: [] }) {
  const url = `${JIKAN_BASE}${endpoint}`;
  return safe(cachedFetchJson(url), fallback);
}
...
export function createApp() {
  ...
  app.get('/api/users/:id/profile', async (req, res) => {
    ...
    const watchTime = entries.reduce((sum, e) => sum + Number(e.progress || 0) * 24, 0);
    res.json({
  ...
  app.get('/api/home', async (_req, res) => {
    ...
    res.json({
      ...
      hiddenGems: (top.data || []).filter((a) => a.score >= 8 && (a.members || 0) < 200000).slice(0, 10).map(normalizeAnime)
    });
  });
  ...
  app.get('/api/anime/search', async (req, res) => {
    ...
    if (season && year) {
      const data = await fetchJikan(`/seasons/${year}/${season}`, { data: [] });
      return res.json({ results: (data.data || []).map(normalizeAnime) });
    }

    const data = await fetchJikan(`/anime?${params.toString()}`, { data: [] });
  ...
  app.get('/api/anime/:id', async (req, res) => {
    ...
    const [anime, chars, recs] = await Promise.all([
      fetchJikan(`/anime/${id}/full`, { data: null }),
      fetchJikan(`/anime/${id}/characters`, { data: [] }),
      fetchJikan(`/anime/${id}/recommendations`, { data: [] })
    ]);
